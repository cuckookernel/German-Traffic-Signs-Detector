...
log = LOGGER.log #pylint: disabel=C0102
...
def test_lenet( model_traits, data, logl=0 ):
    """Train a lenet type NN for image classification"""
    ...
    with tf.Session() as sess:
        ...
        test_accu = tf_evaluate( data["test_4d"], data["test_gt"], batch_size )
...
def tf_evaluate(x_data, y_data, batch_size ):
    ...
    for offset in range(0, num_examples, batch_size):
        ...
        accuracy = sess.run(accu, feed_dict={ images_in: batch_x,
                                              target :  batch_y})
...
def testing() :
    ...
    train_mean = train_4d.mean( axis=(1,2,3), keepdims=True )
    train_x = train_4d - train_mean
    #%%
    #model_traits = MODEL_TRAITS["model2"]
    #valid_log, train_log = train_lenet( train_x, train_gt, test_x, test_gt,
    #                                    model_traits, logl = 1 )
    #%%
    # accu = train_logistic( train_4d, train_gt, verbose=1 )
    #%%
    return  valid_log, train_log, train_x, test_x
